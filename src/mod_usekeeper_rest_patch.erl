...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
-module(mod_usekeeper_rest_patch).
				...
				undefined ->
					case proplists:get_value(response, Data) of
						undefined ->
							...
							content_type_available(Headers, ContentType,
									Body, Path, Resource, ModData);
						_Response ->
							{proceed,  Data}
					end
			end;
...
%% @hidden
content_type_available(Headers, ContentType,
		Body, Uri, Resource, #mod{data = Data} = ModData) ->
			...
			case lists:member(RequestingType, AvailableTypes) of
				true ->
					do_patch(Resource, ModData,
							ContentType, Body, string:tokens(Uri, "/"));
				false ->
...
%% @hidden
do_patch(Resource, #mod{parsed_header = Headers} = ModData, ContentType, Body,
		["usageManagement", "v4", "usageSpecification", UsageSpecId]) ->
	do_response(ModData, Resource:patch_usage_specification(UsageSpecId,
			get_etag(Headers), ContentType, Body));
do_patch(_, #mod{data = Data} = _ModData, _, _, _) ->
	{proceed, Data}.
